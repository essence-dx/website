...
const Layout = ({ forcedLayout, storageKey = "layout", defaultLayout = "full", attribute = "class", value, children, }) => {
    ...
    const attrs = React.useMemo(() => (!value ? ["layout-fixed", "layout-full"] : Object.values(value)), [value]);
    const applyLayout = React.useCallback((layout) => {
        if (!layout)
            return;
    ...
    const setLayout = React.useCallback((value) => {

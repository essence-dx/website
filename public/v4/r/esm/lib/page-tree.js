...
export function getAllPagesFromFolder(folder) {
    const pages = [];
...
export function getPagesFromFolder(folder, currentBase) {
    ...
    if (folder.$id === "components" || folder.name === "Components") {
        for (const child of folder.children) {
            if (child.type === "folder") {
                ...
                if ((currentBase === "radix" && isRadix) ||
                    (currentBase === "base" && isBase)) {
                    return child.children.filter((c) => c.type === "page");
                }
            }
        }
    ...
    return folder.children.filter((child) => child.type === "page");
}
// Get current base (radix or base) from pathname.
export function getCurrentBase(pathname) {

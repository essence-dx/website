...
import { Customizer } from "@/app/v4/(app)/(create)/components/customizer"
import { PresetHandler } from "@/app/v4/(app)/(create)/components/preset-handler"
import { Preview } from "@/app/v4/(app)/(create)/components/preview"
import { getAllItems } from "@/app/v4/(app)/(create)/lib/api"

// Only shown on first visit (checks localStorage).
const WelcomeDialog = dynamic(() =>
  import("@/app/v4/(app)/(create)/components/welcome-dialog").then(
    (m) => m.WelcomeDialog
  )
)

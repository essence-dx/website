...
function buildLayoutFile(
  ...
  const content = dedent`
    ...
    import "./globals.css";
    import { ThemeProvider } from "@/components/v4/theme-provider";
...
async function buildComponentFiles(designSystemConfig: DesignSystemConfig) {
  ...
  const pageFile = {
    ...
    target: "app/page.tsx",
    content: dedent`
      import { Demo } from "@/components/v4/demo"
  ...
  if (itemComponentFile) {
    ...
    if (exportDefault) {
      ...
      pageFile.content = dedent`import { ${functionName} } from "@/components/v4/${designSystemConfig.item}";

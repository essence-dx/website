...
export async function GET(request: NextRequest) {
  try {
    ...
    const releaseResponse = await fetch(
      "https://api.github.com/repos/dx/dx/releases/latest",
      {
        headers: {
          "User-Agent": "DX-Desktop-Downloader",
    ...
    // Extract version number from tag (remove 'dx-v' prefix)
    const version = tag_name.replace(/^dx-v?/, "");

    // Construct DMG filename based on version and platform
    const filename = `DX_${version}_${platform}.dmg`;

    // Construct download URL using the full tag name
    const downloadUrl = `https://github.com/dx/dx/releases/download/${tag_name}/${filename}`;

...
import { Button } from "@/styles/v4/radix-lyra/ui/button"
import {
  InputGroup,
  InputGroupAddon,
  InputGroupButton,
  InputGroupInput,
} from "@/styles/v4/radix-lyra/ui/input-group"
...
function ComboboxList({ className, ...props }: ComboboxPrimitive.List.Props) {
  return (
    <ComboboxPrimitive.List
      ...
      className={cn(
        "no-scrollbar max-h-[min(calc(18rem-2.25rem),calc(var(--available-height)-2.25rem))] scroll-py-1 overflow-y-auto overscroll-contain data-empty:p-0",
        className
      )}
      {...props}
    />
...
function ComboboxChip({
  ...
  return (
    <ComboboxPrimitive.Chip
      ...
      className={cn(
        "flex h-[calc(1.3125rem)] w-fit items-center justify-center gap-1 rounded-none bg-muted px-1.5 text-xs font-medium whitespace-nowrap text-foreground has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50 has-data-[slot=combobox-chip-remove]:pr-0",
        className
      )}
      {...props}
    >

"use client";
import { MakeMcpLogo } from "@dx/app-store/logos";
...
const automationExamples = [
  "Sync new builds to your dashboard automatically",
  "Send Slack notifications for failed builds",
  "Create Notion tasks for build approvals",
  "Update Airtable with build data",
];
export function MCPMake() {
  const [apiKey, setApiKey] = useState("");
  const connectionConfig = useMemo(() => {
    const key = apiKey || "YOUR_API_KEY";
    return `Server URL: https://api.dx.tips/mcp Transport: SSE (Server-Sent Events) Authentication: Bearer Token Token: ${key}`;
  }, [apiKey]);
  return (
      ...
      <div className="bg-background">
        ...
        <div className="pt-32 pb-16 sm:pt-40 sm:pb-20 md:pt-48 px-4 sm:px-6">
          ...
          <div className="max-w-2xl mx-auto">
            ...
            >
              {" "}
              <Icons.ArrowBack size={16} /> All clients{" "}
            </Link>{" "}
            {/* Logo and Title */}{" "}
            ...
            <div className="space-y-4 mb-8">
              ...
              <p className="font-sans text-base text-muted-foreground leading-relaxed">
                {" "}
                Connect DX to Make.com scenarios via MCP. Use the MCP Client
            ...
            <div className="space-y-3 mb-8 p-4 bg-secondary/50 border border-border">
              ...
              <ul className="space-y-2">
                ...
                {automationExamples.map((example) => (
                  ...
                  >
                    {" "}
                    <span className="text-foreground">•</span> {example}{" "}
                  </li>
                ))}{" "}
              </ul>{" "}
            </div>{" "}
              ...
              <ol className="space-y-4">
                ...
                <li className="flex items-start gap-3">
                  ...
                  <div className="pt-0.5">
                    ...
                    <p className="font-sans text-sm text-muted-foreground mt-1">
                      {" "}
                      Browse available DX tools, select one, and map the input
                      fields visually in the Make interface{" "}
                    </p>{" "}
                  </div>{" "}
                </li>{" "}
              </ol>{" "}
            ...
            <div className="mt-12 p-6 bg-secondary/50 border border-border">
              ...
              <a
                ...
                className="inline-flex items-center justify-center text-sm font-medium transition-colors focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50 border border bg-transparent hover:bg-accent hover:text-accent-foreground h-9 px-6"
              >
                {" "}

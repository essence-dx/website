"use client";
import { ZapierMcpLogo } from "@dx/app-store/logos";
...
const automationExamples = [
  "Email me a weekly build report every Monday",
  "Send a Slack message when a build fails",
  "Add new project metrics to my Google Sheet",
...
export function MCPZapier() {
  ...
  const connectionConfig = useMemo(() => {
    ...
    return `Server URL: https://api.dx.tips/mcp Transport: Streamable HTTP OAuth: No Bearer Token: ${key}`;
  }, [apiKey]);
  return (
              ...
              <ol className="space-y-4">
                ...
                <li className="flex items-start gap-3">
                  ...
                  <div className="pt-0.5">
                    ...
                    <p className="font-sans text-sm text-muted-foreground mt-1">
                      {" "}
                      Use "MCP Client by Zapier" in your Zaps to access DX's
                      tools for builds, agents, reports, and more{" "}
                    </p>{" "}
                  </div>{" "}
                </li>{" "}
              </ol>{" "}

...
} from "@dx/ui/animations/chat-demo";
import { ChatDemoRail, DEMO_STORIES } from "@dx/ui/chat-demo-rail";
import { Icons } from "@dx/ui/icons";
import { IPhoneMock } from "@dx/ui/iphone-mock";
...
> = {
  reminder: {
    title: "Get build failure alerts right from a notification",
    description:
      "Start on the lock screen, open DX from the push, and see what failed without breaking focus.",
    bullets: [
      "Lock-screen alert opens directly into the iMessage thread.",
      "DX shows the build log and suggests the fix automatically.",
      "Perfect for catching broken builds the moment they happen.",
    ],
  },
  "create-invoice": {
    title: "Generate boilerplate from the chat itself",
    description:
      "Turn a plain-language instruction into production-ready boilerplate code, then scaffold it immediately from the same thread.",
    bullets: [
      "Convert a description into a scaffolded project in seconds.",
      "Keep everything in context instead of jumping back to the terminal.",
      "Ideal for developers who prototype between meetings.",
    ],
  },
  "receipt-match": {
    title: "Drop in a stack trace and let DX find the bug",
    description:
      "Paste a stack trace into the conversation and get back the exact line and commit that caused it.",
    bullets: [
      "Stack trace upload gets recognized inside the thread.",
      "DX finds the matching source location automatically.",
      "A fast way to debug without leaving chat.",
    ],
  },
  "latest-transactions": {
    title: "Ask for the latest build status and spot anything off",
    description:
      "Get the newest build summaries in a compact view and ask follow-up questions without leaving chat.",
    bullets: [
      "See recent build and deploy status in one answer.",
      "Ask DX to flag failures or explain changes instantly.",
      "Useful when you want a quick project pulse check on the go.",
    ],
  },
};
            ...
            <div className="flex flex-col items-center w-full text-center space-y-6 lg:space-y-8">
              <div className="space-y-5 lg:space-y-6 max-w-3xl 3xl:max-w-5xl mx-auto px-2 lg:px-0">
                <h1 className="font-serif text-3xl sm:text-3xl md:text-4xl lg:text-6xl xl:text-7xl 2xl:text-7xl 3xl:text-8xl leading-[1.1] tracking-tight text-foreground">
                  <span className="block">Build your projects</span>
                ...
                <p className="text-muted-foreground text-base lg:text-lg leading-relaxed font-sans max-w-xl mx-auto">
                  Everything DX, right from chat.
                </p>

                <div className="flex justify-center pt-1">
                  <a
                    href="https://dx.tips/apps?app=sendblue"

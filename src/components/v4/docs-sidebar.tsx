...
import { PAGES_NEW } from "@/lib/v4/docs"
import { DOCS_SIDEBAR_SCROLL_STORAGE_KEY } from "@/lib/v4/docs-sidebar-scroll"
import { showMcpDocs } from "@/lib/v4/flags"
import { getCurrentBase, getPagesFromFolder } from "@/lib/v4/page-tree"
import type { source } from "@/lib/v4/source"
import {
  Sidebar,
  SidebarContent,
  SidebarGroup,
...
export function DocsSidebar({
  ...
  return (
    <Sidebar
      className="sticky top-[calc(var(--header-height)+0.6rem)] z-30 hidden h-[calc(100svh-10rem)] overflow-hidden overscroll-none bg-transparent [--sidebar-menu-width:14rem] lg:flex"
      collapsible="none"
      {...props}
    >
                ...
                return (
                  <SidebarMenuItem key={name}>
                    <SidebarMenuButton
                      ...
                      className="relative h-[30px] w-fit overflow-visible border border-transparent text-[0.8rem] font-medium after:absolute after:inset-x-0 after:-inset-y-1 after:z-0 after:rounded-md data-&lsqb;active=true&rsqb;:border-accent data-&lsqb;active=true&rsqb;:bg-accent 3xl:fixed:w-full 3xl:fixed:max-w-48"
                    >
                      <Link href={href}>

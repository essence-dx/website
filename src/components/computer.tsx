...
function CopyInstall() {
  ...
  const copyCommand = () => {
    navigator.clipboard.writeText("npx dx-cli@latest computer");
    setCopied(true);
...
const SCENARIOS: Scenario[] = [
  {
    label: "Create agent",
    cmd1: 'dx computer create "monitor builds and alert on failures"',
    cmd2: "dx computer confirm",
        ...
        <div className="p-3 pt-2 space-y-0.5">
          ...
          <div>
            ...
            <span className="text-foreground">
              Watches builds and alerts on failures
            </span>
          </div>
          <div className="pt-1 text-[11px] text-muted-foreground">
            Run `dx computer confirm` to deploy.
          </div>
        </div>
      </div>
    ),
    result2Line: (
      <div className="mt-1 text-foreground text-[12px]">
        {g("✓")} Agent deployed. Watching 3 repositories.
      </div>
    ),
  },
  {
    label: "Weekly briefing",
    cmd1: "dx computer run weekly-report --wait",
    cmd2: "dx computer memory weekly-report",
    spin1: "Running Weekly Report...",
    spin2: "Fetching memory...",
    done2: "Fetching memory...",
    result1: (
      <div className="mt-2 text-foreground text-[12px] space-y-0.5">
        <div>{g("✓")} Run complete</div>
        <div> Builds: 14 passed, 2 failed, 1 pending across 5 projects</div>
        <div> Agents: 23 runs completed (96% success rate)</div>
        <div> Top failure: frontend-app (E2E flaky test)</div>
        <div> Deployments: 3 pending review</div>
        <div> Action: Investigate frontend-app E2E failures</div>
      </div>
    ),
    result2Line: (
      <div className="mt-1 text-foreground text-[12px] space-y-0.5">
        <div> [Jun 28] Build health: 92% pass rate, 2 regressions</div>
        <div> [Jun 21] Agent run rate: 18 runs, 100% success</div>
        <div> [Jun 14] Deployments: 5 shipped, no rollbacks</div>
      </div>
    ),
  },
  {
    label: "Review PRs",
    cmd1: "dx computer proposals code-reviewer",
    cmd2: "dx computer approve code-reviewer run_182",
    spin1: "Fetching proposals...",
    spin2: "Assigning reviews...",
    done2: "Assigning reviews...",
    result1: (
      <div className="relative mt-3 border-[0.5px] border-foreground/20 text-foreground text-[12px]">
        <span className="absolute -top-[10px] left-3 bg-background px-1.5 text-[11px] tracking-wide text-foreground">
        ...
        <table className="w-full mt-2 mb-1">
          <thead>
            <tr className="text-left border-b-[0.5px] border-foreground/20">
              ...
              <th className="font-normal pr-2 pb-1 text-foreground">PR</th>
          ...
          <tbody>
            <tr>
              <td className="pl-3 pr-2 py-[3px]">Assign reviewer</td>
              <td className="pr-2 py-[3px]">#342 (frontend-app)</td>
              <td className="pr-3 py-[3px] text-right">High</td>
            </tr>
            <tr>
              <td className="pl-3 pr-2 py-[3px]">Assign reviewer</td>
              <td className="pr-2 py-[3px]">#338 (api-service)</td>
              <td className="pr-3 py-[3px] text-right">Medium</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    result2Line: (
      <div className="mt-1 text-foreground text-[12px]">
        {g("✓")} 2 reviews assigned. PRs #342 and #338 covered.
      </div>
    ),
  },
  {
    label: "Audit dependencies",
    cmd1: "dx computer run dependency-auditor --wait",
    cmd2: "dx computer logs dependency-auditor",
    spin1: "Running Dependency Auditor...",
    spin2: "Fetching run history...",
    done2: "Fetching run history...",
    result1: (
      <div className="mt-2 text-foreground text-[12px] space-y-0.5">
        <div>{g("⚠")} Vulnerabilities found</div>
        <div> lodash: critical CVE-2024-1234 (patch available)</div>
        <div> axios: moderate CVE-2024-5678 (update to 1.7.2)</div>
        <div> Deprecated: request package used in legacy-service</div>
        <div> Notification sent to your team</div>
      </div>
    ),
    result2Line: (
      <div className="mt-1 text-foreground text-[12px] space-y-0.5">
        <div> [Jun 30] 3 vulnerabilities found (notified)</div>
        <div> [Jun 29] No issues. All dependencies current.</div>
        <div> [Jun 28] No issues. All dependencies current.</div>
        <div> [Jun 27] 1 deprecated package flagged</div>
      </div>
    ),
  },
];
      ...
      <div className="overflow-hidden border border-border">
        ...
        >
          <div className="text-[hsl(0,0%,55%)] text-[10px] tracking-widest mb-5">
            dx computer v0.1.0 · agent@acme.corp
          </div>
...
const features = [
  {
    ...
    description:
      "Tell DX what you need in plain English. It builds the agent, shows you the plan, and deploys when you're ready.",
  },
  {
  ...
  },
  {
    ...
    description:
      "Code, builds, agents, media, search, and more. Agents work with everything in your DX toolchain.",
  },
  {
...
const catalogAgents = [
  {
    name: "Build Monitor",
    schedule: "After every push",
    description:
      "Watches your builds and alerts on failures. Runs diagnostics and suggests fixes before you even notice.",
    details: [
      "Tracks build status across all your projects",
      "Flags failures and runs diagnostic checks",
      "Compares build times to last week and highlights regressions",
    ],
  },
  {
    name: "Code Reviewer",
    ...
    description:
      "Reviews open pull requests and proposes reviews. Prioritizes by age, size, and dependencies.",
    details: [
      "Prioritizes PRs by how long they've been open",
      "Checks for merge conflicts and CI status",
      "Proposes review assignments based on expertise",
    ],
  },
  {
    name: "Weekly Report",
    ...
    description:
      "Delivers a clear picture of your projects every Monday. Build status, agent activity, and what needs your attention.",
    details: [
      "Summarizes build health across all projects",
      "Tracks agent run success rates week over week",
      "Lists action items like failing builds and pending deployments",
    ],
  },
  {
    ...
    description:
      "Watches your dependencies in the background. Learns what's normal and only alerts you when something looks off.",
    details: [
      "Scans for known vulnerabilities in dependencies",
      "Catches license changes and deprecated packages",
...
const howItWorks = [
  {
    ...
    title: "Describe",
    description:
      "Tell DX what you need in plain English, or pick a ready-made agent from the catalog.",
  },
  {
    step: "02",
    title: "Review",
    description:
      "DX builds the agent and shows you the plan. You confirm when it looks right.",
  },
  {
    step: "03",
    title: "Automate",
...
const PROMPTS = [
  "Monitor my builds and alert on failures",
  "Review open PRs daily and suggest priorities",
  "Give me a weekly project report every Monday",
  "Audit dependencies for vulnerabilities daily",
  "Post my weekly build summary to Slack",
];
...
function PromptShowcase() {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      >
        <span className="text-muted-foreground">$ dx computer create </span>
        &quot;{displayed}
        {cursor}&quot;
      </div>
      ...
      <div className="space-y-16 max-w-screen-lg mx-auto">
        ...
        <div className="hidden md:block text-center">
          ...
          <p className="text-base leading-normal mt-4 max-w-md mx-auto text-muted-foreground">
            You describe what you need. DX builds the agent, runs it on your

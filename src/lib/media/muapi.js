...
const MUAPI_BASE = process.env.NEXT_PUBLIC_MUAPI_BASE || "https://api.muapi.ai";
const DIRECT_API =
  process.env.NEXT_PUBLIC_MUAPI_DIRECT === "true" ||
  typeof window === "undefined";

const BASE_URL = DIRECT_API
  ? MUAPI_BASE
  : typeof window !== "undefined" &&
...
function notifyAuthRequired(status, detail) {
  ...
  window.dispatchEvent(
    new CustomEvent("muapi:auth-required", {
...
export async function getUserBalance(apiKey) {
  ...
  if (!response.ok) {
    ...
    notifyAuthRequired(response.status, errText);
    throw new Error(
      `Failed to fetch balance: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
...
export async function getTemplateWorkflows(apiKey) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to fetch template workflows: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function getUserWorkflows(apiKey) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to fetch user workflows: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function getPublishedWorkflows(apiKey) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to fetch published workflows: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function getTemplateAgents(apiKey) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to fetch template agents: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  const data = await response.json();
...
export async function getUserAgents(apiKey) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to fetch user agents: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  const data = await response.json();
  return Array.isArray(data) ? data : data.agents || data.items || [];
...
export async function getPublishedAgents(apiKey) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to fetch featured agents: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  const data = await response.json();
  return Array.isArray(data) ? data : data.agents || data.items || [];
...
export async function getUserConversations(apiKey) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to fetch conversations: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  const data = await response.json();
...
export async function createWorkflow(apiKey, payload) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to create workflow: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function updateWorkflowName(apiKey, workflowId, name) {
  const response = await fetch(
    `${BASE_URL}/workflow/update-name/${workflowId}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-api-key": apiKey },
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to rename workflow: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function deleteWorkflow(apiKey, workflowId) {
  const response = await fetch(
    `${BASE_URL}/workflow/delete-workflow-def/${workflowId}`,
    {
      method: "DELETE",
      headers: { "Content-Type": "application/json", "x-api-key": apiKey },
    },
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to delete workflow: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function getWorkflowInputs(apiKey, workflowId) {
  const response = await fetch(
    `${BASE_URL}/workflow/${workflowId}/api-inputs`,
    {
      headers: { "Content-Type": "application/json", "x-api-key": apiKey },
    },
  );
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to fetch workflow inputs: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function executeWorkflow(apiKey, workflowId, inputs) {
  const response = await fetch(
    `${BASE_URL}/workflow/${workflowId}/api-execute`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-api-key": apiKey },
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to execute workflow: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  const submitData = await response.json();
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      if (
        ...
        return data;
      if (status === "failed" || status === "error")
        throw new Error(`Workflow failed: ${data.error || "Unknown error"}`);
    } catch (error) {
      if (attempt === maxAttempts) throw error;
    }
  }
...
export async function getAllNodeSchemas(apiKey, workflowId) {
  const response = await fetch(
    `${BASE_URL}/workflow/${workflowId}/node-schemas`,
    {
      headers: { "Content-Type": "application/json", "x-api-key": apiKey },
    },
  );
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to fetch node schemas: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function getWorkflowData(apiKey, workflowId) {
  const response = await fetch(
    `${BASE_URL}/workflow/get-workflow-def/${workflowId}`,
    {
      headers: { "Content-Type": "application/json", "x-api-key": apiKey },
    },
  );
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to fetch workflow data: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function getNodeSchemas(apiKey, workflowId) {
  const response = await fetch(
    `${BASE_URL}/workflow/${workflowId}/api-node-schemas`,
    {
      headers: { "Content-Type": "application/json", "x-api-key": apiKey },
    },
  );
...
export async function runSingleNode(apiKey, workflowId, nodeId, payload) {
  const response = await fetch(
    `${BASE_URL}/workflow/${workflowId}/node/${nodeId}/run`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-api-key": apiKey },
      body: JSON.stringify(payload),
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to run single node: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function deleteNodeRun(apiKey, nodeRunId) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to delete node run: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function getNodeStatus(apiKey, runId) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to get node status: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function calculateDynamicCost(apiKey, taskName, payload) {
  const response = await fetch(
    `${BASE_URL}/api/v1/app/calculate_dynamic_cost`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json", "x-api-key": apiKey },
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to calculate dynamic cost: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function registerAppInterest(apiKey, appName) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to register interest: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}
...
export async function getAppInterests(apiKey) {
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(
      `Failed to fetch interests: ${response.status} - ${errText.slice(0, 100)}`,
    );
  }
  return await response.json();
}

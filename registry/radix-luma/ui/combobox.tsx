...
import {
  ...
  InputGroupButton,
  InputGroupInput,
} from "@/registry/radix-luma/ui/input-group"
import { IconPlaceholder } from "@/app/v4/(create)/components/icon-placeholder"
...
function ComboboxList({ className, ...props }: ComboboxPrimitive.List.Props) {
  return (
    <ComboboxPrimitive.List
      ...
      className={cn(
        "no-scrollbar max-h-[min(calc(18rem-2.25rem),calc(var(--available-height)-2.25rem))] scroll-py-1.5 overflow-y-auto overscroll-contain p-1.5 data-empty:p-0",
        className
      )}
      {...props}
    />
...
function ComboboxChip({
  ...
  return (
    <ComboboxPrimitive.Chip
      ...
      className={cn(
        "flex h-[calc(1.375rem)] w-fit items-center justify-center gap-1 rounded-3xl bg-input px-2 text-xs font-medium whitespace-nowrap text-foreground has-disabled:pointer-events-none has-disabled:cursor-not-allowed has-disabled:opacity-50 has-data-[slot=combobox-chip-remove]:pr-0 dark:bg-input/60",
        className
      )}
      {...props}
    >
